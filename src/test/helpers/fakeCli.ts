/**
 * Fake CLI helper for creating mock tsqlrefine executables in tests.
...
/**
 * Creates a fake tsqlrefine CLI executable for testing.
...
export async function createFakeCli(scriptBody: string): Promise<FakeCli> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "tsqlrefine-fake-"));
	const scriptPath = path.join(dir, "fake-tsqlrefine.js");
	...
	if (process.platform === "win32") {
		commandPath = path.join(dir, "fake-tsqlrefine.cmd");
		const cmd = `@echo off\r\n"${nodePath}" "${scriptPath}" %*\r\n`;
		await fs.writeFile(commandPath, cmd, "utf8");
	} else {
		commandPath = path.join(dir, "fake-tsqlrefine");
		const sh = `#!/bin/sh\n"${nodePath}" "${scriptPath}" "$@"\n`;
